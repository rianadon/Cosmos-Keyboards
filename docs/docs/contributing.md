...
5. Go back to `src/lib/geometry/microcontrollers.ts` and add the model url under `glbFile`. The glb file will be placed in `target` or `src/assets` depending on whether you autogenerated the board or not.
6. To auto-assign a specific connector when choosing the microcontroller in basic mode, edit `microControllerConnectors` in the same file. By default, microcontrollers with _Bluetooth_ in their `extraName` are assigned only a usb connector, while all others are assigned USB & TRRS.
