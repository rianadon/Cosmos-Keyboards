...
export function makeHand(hand: PoseHand): Hand {
	const vectors = hand.keypoints3D.map((a) => new Vector3(a.x, -a.y, a.z));
	const handedness = hand.handedness;
	const score = hand.score;

	const transform = makeBasis(vectors, handedness === 'Right').invert();
	...
	return { hand, vectors, handedness, score, limbs };
}
...
export function averageNorms(v: Vector3[], length: number): Joint {
	const position = v
	...
	return { length, position, V, Vinv, degree: 0 };
}
