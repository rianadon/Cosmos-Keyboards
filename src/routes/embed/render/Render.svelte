<script lang="ts">
  ...
  import { CASE_COLOR, KEY_COLOR, PLATE_COLOR } from './defaults'
...
{#each models as [kbd, meshes] (kbd)}
  ...
  {#if cent}
    <T.Group position={[-cent[0], -cent[1], -cent[2]]} scale.x={kbd == 'left' ? -1 : 1}>
      ...
      <Keyboard geometry={geometry[kbd] || null} flip={kbd == 'left'} side={kbd} {keyColor} />
    </T.Group>
  {/if}
{/each}

<T.DirectionalLight position={[0, 0, 10]} intensity={0.8} />
<T.DirectionalLight position={[0, 5, 5]} intensity={0.8} />
<T.AmbientLight args={[0xffffff]} intensity={1.5} />
