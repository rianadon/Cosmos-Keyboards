...
interface CuttleKeycapKey extends CuttleBaseKey {
  type: 'old-box' | 'old-mx' | 'mx-better' | 'old-mx-snap-in' | 'alps' | 'choc' | 'mx-pcb' | 'old-mx-hotswap' | 'old-mx-snap-in-hotswap' | 'mx-better-pcb' | 'choc-hotswap' | 'mx-hotswap' | 'mx-sunken' | 'mx-sunken-twist' | 'choc-sunken'
  keycap: Keycap
}
...
export function switchType(c: DeepRequired<CuttleformProto>): Required<CuttleKeycapKey>['type'] {
  ...
  if (c.upperKeys.switchType == SWITCH.MX_BETTER_PCB) return 'mx-better-pcb'
  return 'box'
}
