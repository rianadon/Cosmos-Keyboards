<script>
  /**
   * Canvas is modified slightly from svelte-cubed.
   * There seems to be a bug (well-intentioned or not) where the canvas's width
   * and height are set to be independent of the device pixel ratio.
   * Instead, these dimensions should scale with the pixel ratio.
   */
  ...
  /** @type {boolean | typeof THREE.BasicShadowMap | typeof THREE.PCFShadowMap | typeof THREE.PCFSoftShadowMap | typeof THREE.VSMShadowMap | undefined} */
