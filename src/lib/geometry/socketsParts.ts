...
export const PART_INFO: Record<CuttleKey['type'], PartInfo> = {
  ...
  'meh01': {
    ...
    decodeVariant: (variant: number) => {
      return {
        sw: ['switch', 'switchless'][variant] || 'switch',
      }
    },
  },
  ...
  'trackball': {
    ...
    variants: {
      size: ['25mm', '34mm', '43mm', '46mm', '55mm'],
      bearings: ['Roller', 'Ball', 'BTU (7.5mm)', 'BTU (9mm)'],
      sensor: ['Joe (QMK)', 'Skree (ZMK)'],
    },
    encodeVariant: (variant: Variant) => {
      const size = ['34mm', '25mm', '55mm', '43mm', '46mm'].indexOf(variant.size)
    ...
    decodeVariant: (variant: number) => {
      ...
      return {
        size: ['34mm', '25mm', '55mm', '43mm', '46mm'][size] || '34mm',
