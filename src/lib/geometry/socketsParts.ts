...
export const PART_INFO: Record<CuttleKey['type'], PartInfo> = {
  ...
  'meh01': {
    ...
    numPins: (v: Variant) => ({ matrix: v.sw == 'switch' ? 1 : 0, gpio: 2 }),
    icon: 'knob',
    description:
  ...
  'oled-128x32-0.91in-spi-adafruit': {
    ...
    partBottom: [box(22, 33.2, 4.1)],
    numPins: { spi: 'input-only', gpio: 1 }, // SPI + 1 DC pin
    icon: 'oled',
    description: DESC_DISPLAY,
  },
  ...
  'oled-168x144-1.26in-keydio-vista508': {
    partName: 'Keydio Vista508 168x144 1.26" OLED',
    bomName: 'Keydio Vista508 168x144 1.26" OLED',
    category: 'Displays',
    stepFile: '/target/key-oled-168x144-1.26in-keydio-vista508.step',
    socketSize: [24.85, 36.1, 1.8],
    partBottom: [box(24.75, 36, 2.8)],
    numPins: { spi: 'input-only' },
    icon: 'oled',
    description: DESC_DISPLAY,
  },
  ...
  'trackball': {
    ...
    socketSize: (v: Variant) => ({
      radius: parseFloat(v.size) / 2 + 3.9,
      height: 4,
      sides: 20,
    }),
    partBottom: (v: Variant) => {
      ...
      const bottom = parseFloat(v.size) / 2 + 11.5
      return [box(pcbWidth, pcbHeight, bottom), box(16, 11, bottom + 2.5)]
    },
    variants: {
      size: ['25mm', '34mm', '40mm', '43mm', '44mm', '45mm', '46mm', '50mm', '52.5mm', '55mm', '57.15mm'], // add new variants in numerical order
      bearings: ['Roller', 'Ball', 'BTU (7.5mm)', 'BTU (9mm)'],
      sensor: ['Joe (QMK)', 'Skree (ZMK)'],
    },
    encodeVariant: (variant: Variant) => { // add new variants to the end
      const size = ['34mm', '25mm', '55mm', '43mm', '46mm', '45mm', '40mm', '44mm', '50mm', '52.5mm', '57.15mm'].indexOf(variant.size)
      ...
      return (size & 7) + (bearings << 3) + (sensor << 6) + ((size >> 3) << 8)
    },
    decodeVariant: (variant: number) => {
      const size = (variant & 0x7) | (((variant >> 8) & 3) << 3)
      const bearings = (variant >> 3) & 0x7
      const sensor = (variant >> 6) & 0x3
      return {
        size: ['34mm', '25mm', '55mm', '43mm', '46mm', '45mm', '40mm', '44mm', '50mm', '52.5mm', '57.15mm'][size] || '34mm',
    ...
    numPins(v: Variant) {
      const variant = v as TrackballVariant
      if (variant.sensor == 'Skree (ZMK)') return { spi: 'output-only', gpio: 1 } // SPI + Motion trigger
...
type Pins = {
  ...
  matrix?: number
  /** Number of pins that must be connected to adc. */
  analog?: number
  /** Number of pins that must be attached to GPIO pins, minus matrix + SPI + I2C + analog pins. */
  gpio?: number
  /** Specify this if the part connects to the SPI bus. Displays are "input-only". Trackballs might be "output-only" */
  spi?: 'bidirectional' | 'output-only' | 'input-only'
  i2c?: boolean
}
