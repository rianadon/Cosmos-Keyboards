...
import type { CrossSection, Manifold, ManifoldToplevel, Mesh, Vec2, Vec3 } from 'manifold-3d'
...
type Shape = Manifold | CrossSection | null
...
class MyVisitor {
  visitModuleInstantiationStmt(n: ModuleInstantiationStmt): Shape {
    ...
    switch (n.name) {
      ...
      case 'multmatrix': {
        ...
        for (let i = 0; i < 4; i++) {
          return operands!.transform(transpose.flat())
        }
      }
      case 'cube': {
  ...
  private findRadius(args: AssignmentNode[]) {
  ...
  private getFragmentsFromR(r: number | number[], args: AssignmentNode[]) {
  ...
  private processBlock(fn: (s: Shape[]) => Shape, stmt: Statement): Shape {
    if (!(stmt instanceof BlockStmt)) throw new Error('Expected a block')
    const shapes: Shape[] = []
    for (const s of stmt.children) {
      if (s instanceof NoopStmt) continue
      if (s instanceof ModuleInstantiationStmt) shapes.push(this.visitModuleInstantiationStmt(s))
      else throw new Error('Unknown stmt')
    }
    return fn(shapes)
  }

  visitScadFile(n: ScadFile): BufferGeometry {
    const shapes: Shape[] = []
    for (const stmt of n.statements) {
      if (stmt instanceof ModuleInstantiationStmt) shapes.push(this.visitModuleInstantiationStmt(stmt))
      else throw new Error('Unknown stmt')
    }
